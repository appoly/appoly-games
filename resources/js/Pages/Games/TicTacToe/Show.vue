<template>
<Head :title="'Game ' + tic_tac_toe.id" />
    <AuthenticatedLayout>
        ...
        <ul class="max-w-sm mx-auto mt-6 space-y-2">
            <li class="flex items-center gap-2">
                <span class="p-1.5 font-bold rounded bg-gray-200" :class="{'bg-green-200': xTurn}">X{{ xTurn ? ' turn' : ''}}</span>
                <span>{{ tic_tac_toe.player_one.name }}</span>
                <span :class="{ '!bg-green-500': players.find(({id}) => id === tic_tac_toe.player_one_id) }" class="bg-red-500 size-4 rounded-full"></span>
            </li>
            <li v-if="tic_tac_toe.player_two" class="flex items-center gap-2">
                <span class="p-1.5 font-bold rounded bg-gray-200" :class="{'bg-green-200': !xTurn}">O{{ !xTurn ? ' turn' : ''}}</span>
                <span>{{ tic_tac_toe.player_two.name }}</span>
                <span :class="{ '!bg-green-500': players.find(({id}) => id === tic_tac_toe.player_two_id) }" class="bg-red-500 size-4 rounded-full"></span>
            </li>
...
const props = defineProps(['tic_tac_toe']);

const boardState = ref(props.tic_tac_toe.state ?? [0, 0, 0, 0, 0, 0, 0, 0, 0]);
...
const yourTurn = computed(() => {
    if (props.tic_tac_toe.player_one_id === page.props.auth.user.id) {
        return xTurn.value;
    }
...
const channel = Echo.join(`room.${props.tic_tac_toe.id}`)
...
onUnmounted(() => {
    Echo.leave(`room.${props.tic_tac_toe.id}`);
});

function updateOpponent() {
    router.put(route('games.tic-tac-toe.update', props.tic_tac_toe.id), {
